...
subplot(2,2,2);imshow(R1/250);  %显示变换后的图像
title('小波变换后图像');axis('square');
subplot(2,2,3);imshow(watermark1*10^16);  %显示水印图像
axis('square');title('水印图像');
%水印检测
...
subplot(2,2,4);plot(correlation1,'-.k');
hold on;
plot(originalthresholdvector,'-r');
title('原始的加水印图像');
xlabel('水印');ylabel('检测响应')
